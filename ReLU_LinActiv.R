...
rel_forward_propagation<-function(X,parameters,leaky){
  W1<-parameters$W1
...
rel_compute_cost<-function(A2,Y){
  m<-dim(Y)[2]
...
rel_backward_propagation<-function(parameters,cache,X,Y,leaky){
  m<-dim(Y)[2]
  ...
  #dZ1<-(t(W2)%*%dZ2)*(1-A1^2)
  print(dim(t(W2)%*%dZ2))
  print(dim(RelU_grad(Z1,leaky)))
  dZ1<-(t(W2)%*%dZ2)*t(RelU_grad(Z1,leaky))
  dW1<-dZ1%*%t(X)/m
...
rel_nn_model<-function(X, Y, n_h, num_iterations = 10000, print_cost=F,learning_rate = 1.2,leaky=0){
  set.seed(3)
  ...
  for (i in 1:num_iterations){
    ...
    # cost
    cost<-rel_compute_cost(cache[["A2"]],Y)
    # backpropagation
    grads<-rel_backward_propagation(parameters,cache,X,Y,leaky)
