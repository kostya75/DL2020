...
initialize_parameters_deep<-function(layer_dims,X_col=209){
  set.seed(3) #comment out
  parameters<-list()
  L<-length(layer_dims)
  for(l in 2:L){
    parameters[[sprintf("W%s",l-1)]]<-matrix(rnorm(layer_dims[l-1]*layer_dims[l])*.01,ncol=layer_dims[l-1])
    parameters[[sprintf("b%s",l-1)]]<-matrix(0,nrow=layer_dims[l],ncol=X_col)
  }
  return(parameters)
}
...
# test
parameters<-initialize_parameters_deep(c(5,4,3),209)
...
b<-matrix(c(-0.24937038,-0.24937038),nrow=dim(W)[1])
...
b<-matrix(c(-0.90900761,-0.90900761),nrow=dim(W)[1])
...
b2<-matrix(c( 1.50278553,
              -0.59545972,
              0.52834106,1.50278553,
              -0.59545972,
              0.52834106,1.50278553,
              -0.59545972,
              0.52834106,1.50278553,
              -0.59545972,
              0.52834106),ncol=4,byrow=F)

W3<-matrix(c( 0.9398248 ,  0.42628539, -0.75815703),nrow=1,byrow=T)
b3<-matrix(c(-0.16236698,-0.16236698,-0.16236698,-0.16236698),nrow=1,byrow=F)
...
linear_cache<-list(
  ...
  b=matrix(c(2.10025514,
             0.12015895,
             0.61720311,2.10025514,
             0.12015895,
             0.61720311,2.10025514,
             0.12015895,
             0.61720311,2.10025514,
             0.12015895,
             0.61720311),ncol=4,byrow=F)
)
...
caches<-list(
  list(
    linear_cache=list(A=matrix(c(  0.09649747, -1.8634927  ,
                      ...
                      b=matrix(c(  1.48614836 ,
                                   0.23671627 ,
                                   -1.02378514,1.48614836 ,
                                   0.23671627 ,
                                   -1.02378514   ), nrow=3,byrow=F)),
  ...
  list(
    linear_cache=list(A=matrix(c(  1.97611078, -1.24412333 ,
                      ...
                      b=matrix(c( -1.62328545,-1.62328545   ), nrow=1,byrow=F)),
...
parameters<-list(W1=matrix(c(-0.41675785, -0.05626683, -2.1361961 ,  1.64027081,
                 ...
                 b1=matrix(c( 0.04153939,
                              -1.11792545,
                              0.53905832,0.04153939,
                              -1.11792545,
                              0.53905832), ncol=2,byrow=F),
                 W2=matrix(c(-0.5961597 , -0.0191305 ,  1.17500122), nrow=1,byrow=T),
                 b2=matrix(c(-0.74787095,-0.74787095), nrow=1,byrow=F))
...
grads<-list(list(dW1=matrix(c( 1.78862847,  0.43650985,  0.09649747, -1.8634927  ,
                 ...
                 db1=matrix(c(0.88131804 ,
                              1.70957306 ,
                              0.05003364,0.88131804 ,
                              1.70957306 ,
                              0.05003364), ncol=2, byrow=F)),
            list(dW2=matrix(c(-0.40467741, -0.54535995, -1.54647732), nrow=1, byrow=T),
                 db2=matrix(c(0.98236743,0.98236743), nrow=1, byrow=F))
)
